...
本版本是对标 oh-my-opencode v3.17.14 的全面升级，涵盖 8 个阶段的系统重构，新增 51 个文件（35 新建 + 16 修改）。

This release is a comprehensive upgrade aligned with oh-my-opencode v3.17.14, spanning 8 phases of systematic refactoring with 51 files changed (35 new + 16 modified).

---

### 🎯 Phase 1: Background Agent System / 后台 Agent 系统

- **BackgroundTaskManager** (`lib/background/manager.ts`) — 程序化后台任务生命周期管理
  - 文件锁信号量并发控制（默认最大 5 并发）
  - 指数退避自动重试：`min(2^attempt × 10, 300)` 秒
  - 基于 JSON 文件的状态持久化 (`~/.oh-my-claude/background-tasks/`)
- **New Hooks**:
  - `background-task-launcher.sh` — UserPromptSubmit: 检测 `background_task(...)` 模式并注册任务
  - `background-task-tracker.sh` — PostToolUse: 追踪 Task/Agent 工具调用
  - `background-task-retry.sh` — PostToolUse: 失败任务指数退避重试
- **Enhanced**: `background-notification.sh` 集成 TypeScript CLI，显示详细的完成/错误状态

### 🏔️ Phase 2: Boulder Enhancement / 巨石机制增强

- **EnforcerStateMachine** (`lib/todo/enforcer-state.ts`) — 状态机驱动的 todo 继续执行器
  - 停滞检测：连续 3 次无进展 → 停止继续
  - 压缩冷却：压缩后 60 秒内禁止继续
  - 指数退避：`min(2 × 2^failures, 120)` 秒
  - 最大连续失败：5 次
- **Rewritten**: `todo-continuation-enforcer.sh` 集成状态机、进度条、abort/token-limit 检测
- **New**: `hooks/lib/enforcer-helpers.sh` — 共享 bash 函数库

### 🗺️ Phase 3: Atlas Orchestration Engine / Atlas 编排引擎

- **BoulderStateManager** (`lib/atlas/boulder-state.ts`) — 巨石计划状态持久化
- **Session Lineage** (`lib/atlas/session-lineage.ts`) — 追加式会话血缘日志
- **Continuation Injector** (`lib/atlas/continuation-injector.ts`) — 构建包含 `PLAN_NAME`、`REMAINING/TOTAL` 的继续提示
- **Approval Gate** (`lib/atlas/approval-gate.ts`) — 最终 wave 审批检测
- **Verification Reminders** (`lib/atlas/verification-reminders.ts`) — 波次后验证提醒
- **Rewritten**: `atlas.sh` 集成巨石状态和置信度分类

### #️⃣ Phase 4: Hashline Enhancement / Hashline 增强

- **Hash Computation** (`lib/hashline/hash-computation.ts`) — 基于 MD5 的 LINE#ID 哈希算法
  - 256 符号字典索引格式：`N#HASH|content`
  - 结构化 filediff 元数据（additions/deletions/diff）
- **Edit Validator** (`lib/hashline/validation.ts`) — 编辑前验证哈希值是否匹配当前文件内容
- **New Hook**: `hashline-edit-validator.sh` — PreToolUse: 拒绝使用过期哈希的 Edit

### 🔄 Phase 5: Model Fallback Enhancement / 模型回退增强

- **ModelFallbackController** (`lib/model-fallback/state-controller.ts`) — 运行时自动模型切换
- **Error Classifier** (`lib/model-fallback/error-classifier.ts`) — 分类为 retryable/stop/non_retryable
- **Compatibility Enforcer** (`lib/model-fallback/compatibility-enforcer.ts`) — Agent↔模型兼容性检查
- **Rewritten**: `model-fallback.sh` 集成 TypeScript CLI 和错误分类

### 🪝 Phase 6: Missing Hooks / 补全 Hooks

- `read-image-resizer.sh` — PreToolUse: 使用 ImageMagick/ffmpeg 缩放 >2048px 的图片
- `tasks-todowrite-disabler.sh` — PreToolUse: 阻止特定 Agent/命令使用 TodoWrite
- `tool-pair-validator.sh` — PostToolUse: 检测冲突工具对（Write+Edit 同一文件）
- `anthropic-effort.sh` — UserPromptSubmit: 检测 "think harder" 关键词，注入 effort 指令

### 📦 Phase 7: New Skills / 新技能

- `skills/github-triage/` — 只读 Issue/PR 分类技能
- `skills/work-with-pr/` — 完整 PR 生命周期自动化
- `skills/pre-publish-review/` — 多 Agent 发布质量门控

### 🔧 Phase 8: Infrastructure / 基础设施

- **Bun 构建** (`bunfig.toml`, `script/build-binaries.ts`) — 5 平台原生二进制文件
  - linux-x64, linux-arm64, darwin-x64, darwin-arm64, windows-x64
- **构建脚本**: `build:schema` — JSON Schema 生成
- **依赖更新**: `@types/node` ^25.0.9, `typescript` ^5.9.3, `zod` ^3.22.4

### 📊 统计 / Statistics

- **新增文件**: 35 个（TypeScript 模块、Hook 脚本、Skill 定义）
- **修改文件**: 16 个（hooks、package.json、hooks.json）
- **新增 Hook**: 10 个
- **新增 Skill**: 3 个
- **新增 CLI 模块**: 5 个（background、todo、atlas、hashline、model-fallback）
- **TypeScript 编译**: 零错误
- **架构**: 复杂逻辑提取到 `lib/` TypeScript 模块，bash hooks 通过 `node dist/lib/<module>/cli.js` 调用
...
#### 🤖 智能模式自动选择 / Smart Mode Auto-Selection

**无需手动选择模式**，系统根据任务特征自动选择最佳执行模式：

| 任务特征 | 自动选择 | 触发示例 |
|----------|----------|----------|
| 多组件/全栈 | ⚡ 超级模式 | "构建全栈应用" "前端+后端" |
| 批量/同类 | 🐝 蜂群模式 | "修复所有错误" "为每个组件添加" |
| 简单/省Token | 💰 节俭模式 | "简单修改" "省token" |
| 通用任务 | ⛰️ 标准模式 | 默认 |

- 新增命令：`/auto`, `/smart`, `/zhineng`
- 关键词检测器自动识别任务特征并路由到对应模式
- 模式优先级：超级 > 蜂群 > 节俭 > 标准

### 🐛 Fixed / 修复

- 修复 `/auto` 命令单独使用时无法匹配的问题
